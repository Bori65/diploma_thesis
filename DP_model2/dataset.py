...
class MultimodalSimulation(Dataset):
    ...
    def __getitem__(self, item):
        ...
        joints = torch.zeros(self.max_frames, 6, dtype=torch.float32)
        if self.precooked:
            #frames = torch.zeros(num_frames, self.feature_dim, dtype=torch.float32)
            frames = torch.zeros(self.max_frames, self.feature_dim, dtype=torch.float32)
        else:
            #frames = torch.zeros(num_frames, 3, 224, 398, dtype=torch.float32)  # img shape (3, 224, 398)
            frames = torch.zeros(self.max_frames, 3, 224, 398, dtype=torch.float32)  # img shape (3, 224, 398)
        ...
        for i, frame_number in enumerate(frame_numbers):
            ...
            try:
                ...
                print(f"Error loading joint number: {sequence_number} frame: {i}")
            ...
            if self.precooked:
                ...
                try:
                    ...
                    print(f"Error loading image number: {sequence_number} frame: {i}")
                    frames[i] = torch.zeros((3, 224, 398), dtype=torch.float32)
    ...
    def get_sentence_string(self, label):
        return f"{self.DICTIONARY[int(label[0])]} {self.DICTIONARY[int(label[1])]} {self.DICTIONARY[int(label[2])]}"
